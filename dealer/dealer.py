...
from view_model.view_dealer import ViewDealer
...
#create a logger and write to dealer log
CUR_DIR = os.path.dirname(os.path.realpath(__file__))
LOG_FILE = os.path.join(CUR_DIR, "log_dealer")
...
class Dealer:
    ...
    def run_turn(self):
        ...
        logger.info("Player States: %s \n WH: %s \n" % (self.player_states, self.wateringhole))
    ...
    def get_all_card_choices(self):
        ...
        :return: list of tuples containing both choices
        :rtype: ([Nat, ...], [[CardPlay, ...], ...])
        """
        food_card_choices = []
        card_plays = []
        for player_state in self.player_states:
            try:
                ...
                logger.info("Food Card choices for player %s: %s \n" % (player_state.id, f_c_i))
                logger.info("Card Plays: %s" % c_p)
    ...
    def apply_card_plays(self, lolocp):
        ...
        :return: None
        """
        logger.info("Apply card plays\n")
        for player_state, locp in zip(self.player_states, lolocp):
            logger.info("player_state before: %s" % player_state)
            for cp in locp:
                cp.apply(player_state)
            logger.info("player_state after: %s\n" % player_state)
    ...
    def before_feeding_cycle(self):
        ...
        logging.info("species list before feeding cycle \n")
        for player in self.player_states:
            logger.info("Player: %s, Species: %s" % (player.id, player.species_list))
        logging.info("\n")
    ...
    def view_model(self):
        """
        creates a view model for this dealer
        :rtype ViewDealer
        """
        return ViewDealer(self)
